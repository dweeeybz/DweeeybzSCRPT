...
local function BuildListTab(tabObj, kind, dataTable, categories, categoryOrder, equipFn, favSet)
	...
	local function toggleFavorite(name)
		favSet[name] = not favSet[name] or nil
		if RefreshFavoritesTab then RefreshFavoritesTab() end
	end
			...
			if items and #items > 0 then
				...
				if #matched > 0 then
					...
					for _, itemName in ipairs(matched) do
						...
						local favBtn = Section:Button({
							Title = isFavorite(itemName) and "★ Remove Favorite" or "☆ Add Favorite",
							Icon = "star",
							Callback = function()
						...
						table.insert(dynamicElements, favBtn)
					end
				end
			end
		end
