...
local Emotes = {
	['Jamal Brazil Groove'] = 104131847054135;
	['Guap Dance Trend'] = 119681336967955;
	['Fashion'] = 3333331310; 
...
local Window = WindUI:CreateWindow({
	...
	KeySystem = {
		...
		KeyValidator = function(key)
			if key == "imongmama" then
				return true
			end
			WindUI:Notify({ Title = "Key System", Content = "fail", Duration = 3, Icon = "x" })
			return false, "fail"
		end,
	},
	Author = "DweeybzSCRPT",
...
local function BuildListTab(tabObj, kind, dataTable, categories, categoryOrder, equipFn, favSet)
	...
	-- Builds only the matching buttons. filterText == nil means "don't render
	-- anything yet" (used on first load so we don't create 100s of buttons
	-- before the window has even opened -- that's what was causing the lag).
	local function Rebuild(filterText)
		ClearElements(dynamicElements)

		if filterText == nil then
			local PromptSection = tabObj:Section({ Title = "Browse " .. kind .. "s" })
			table.insert(dynamicElements, PromptSection)
			local hint = PromptSection:Button({
				Title = "Type in the search bar above, or tap here to load all " .. kind:lower() .. "s",
				Icon = "search",
				Callback = function() Rebuild("") end,
			})
			table.insert(dynamicElements, hint)
			return
		end
	...
	Rebuild(nil) -- show the lightweight prompt instead of the full list on load
	return Rebuild
end
